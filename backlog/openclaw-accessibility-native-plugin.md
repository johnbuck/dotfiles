...
- **registers-a11y-audit-tool** — `register(api)` calls `api.registerTool` once with `name: "a11y_audit"` and a parameters schema exposing `url`, `html`, `standard`; verified by: unit test `register registers tool` using a fake `api` that records the registration.
...
**Tool-registration API (verified against the running OpenClaw 2026.4.24 SDK and both in-repo sibling plugins):** the entry exports a default object with a `register(api)` method (NOT `setup` — every in-tree tool extension and both dotfiles siblings use `register`); tools are registered with `api.registerTool({ name, label, description, parameters, async execute(toolCallId, params) {…} })`. `parameters` is a plain JSON-Schema object literal, not TypeBox — this keeps the module loadable with zero third-party packages so `node --test` runs dependency-free. (An earlier draft of this spec said `setup`/TypeBox; corrected after the e2e verification pass.)
