...
- **pre-build** — you have the spec and the planner's design (approach + success criteria), but NO code has been written yet. Catch drift before a single token is spent building: is this spec on the roadmap? Does the plan honor the North Star and the project's principles? Has the planner widened scope beyond what the spec/roadmap calls for? This pass is cheap and prevents building the wrong thing correctly. **It also gates the spec itself** — drift usually enters the pipeline as a spec defect, so verify: (a) a spec that modifies an existing surface carries an as-is inventory of that surface (each filter/limit/guard/gate/branch with file refs) and a KEEP/CHANGE/REMOVE change map — spot-check the inventory against the real code for missing elements; (b) its North Star check quotes the North Star / canonical-reference rules governing the surface verbatim (+ compliance + a testable condition) — read the canon for that surface yourself and flag governing principles the spec omits, AND flag any spec-invented "rule" with no North Star/canonical source (a spec-local rule has no authority and can enshrine a bug; a missing rule is a North Star update for the operator, never a spec-local declaration); (c) every canon quote in the spec matches the cited file — a "canonical" claim that doesn't match its source is a High finding (fabricated canon is the worst drift vector).
