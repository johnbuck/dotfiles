...
| `hooks/cognee-remind.sh` | **cognee-first reminder** (UserPromptSubmit, wired in `settings.json`) — appends the recall-first rule to every turn; suppresses itself inside a `*/Bismouth/homelab*` checkout, which carries its own project-level hook. A one-shot PreToolUse gate (`cognee-gate.sh`) denied the session's first research call until recall ran; removed 2026-09-15 by operator decision — the reminder is the whole mechanism now. Twins ship for OpenCode (`../opencode/plugins/pnk-cognee-first.js`) and pi (`../pi/extensions/cognee-first.ts`). |
