...
| `workflows/`, `commands/`, `hooks/pnk-baton-guard.sh` | **pnk-baton** — a multi-agent build/review pipeline (`/pnk-baton <spec>`): plan → test-first → build → review → auto-merge, each run in its own git worktree. See [`PNK-BATON.md`](./PNK-BATON.md). |
| `hooks/cognee-{remind,gate}.sh` | **cognee-first**, two layers wired in `settings.json`. `cognee-remind.sh` (UserPromptSubmit) appends the recall-first rule to every turn; `cognee-gate.sh` (PreToolUse) denies the session's *first* research tool call unless recall already ran — one denial per session, fail-open, self-defusing when cognee isn't available. The reminder suppresses itself inside a `*/Bismouth/homelab*` checkout, which carries its own project-level hook. Twins of both ship for OpenCode (`../opencode/plugins/pnk-cognee-first.js`) and pi (`../pi/extensions/cognee-first.ts`). |
| `hooks/spec-standard-reminder.sh` | Non-blocking PreToolUse (Write\|Edit) reminder on `backlog/*.md`: new specs should go through the `pnk-spec` skill and carry the as-is inventory, change map and acceptance criteria that pnk-baton's Align gate checks. Impl-logs, canonicals, handoffs, STATUS and ROADMAP files are exempt. |
...
| `plugins/local/pnk-plan/` | Local planning plugin: the `pnk-spec`, `pnk-roadmap` and `pnk-scaffold` skills that pnk-baton consumes. Supersedes the old `do-the-thing` plugin, which `install.sh` retires if a prior install left it behind. |
