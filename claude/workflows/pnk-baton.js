...
const plan = await agent(
  `You are the pnk-baton PLANNER.\n${fields}\n\nRead the spec, ground the design in the actual code, and produce a testable plan. Identify the smallest change that satisfies the requirement.\n\nCarry the spec's INVARIANTS into the plan: include each rule from the spec's "Invariants" section as its own success criterion prefixed "invariant:" (testable as stated there), so every downstream stage — tests, reviewers, validator — enforces what must NOT change alongside what must. Honor the spec's change map: anything it marks KEEP is off-limits to the design; only its REMOVE lines authorize deleting existing behavior. If the spec modifies an existing surface but lacks an as-is inventory / change map / Invariants section, verify the current behavior against the code yourself and list what's missing in openQuestions — do not fill the gap by silent assumption.`,
...
const align = await agent(
  `You are the pnk-baton DRIFT-CHECKER in **pre-build** mode. No code exists yet — judge the SPEC and the PLANNER's design for alignment before any build tokens are spent.\n${fields}\n\n${driftNote}${envNote}\n\nPlanner summary: ${plan.summary}\nPlanner approach:\n${plan.approach}\nSuccess criteria:\n${plan.successCriteria.map((c, n) => `${n + 1}. ${c}`).join('\n')}\n\nFIRST, gate the SPEC's structure (this is where drift enters the pipeline):\n1. **As-is + change map** — if the spec modifies an existing surface, it must inventory that surface's current behavior (each filter/limit/guard/gate/fallback/branch, with file refs) and disposition each element KEEP/CHANGE/REMOVE. Spot-check the inventory against the actual code: an element that exists on the path but is missing from the inventory is a High finding (axis: spec) — the build would guess its fate. A spec that changes existing behavior with NO as-is inventory at all is DRIFT.\n2. **Invariants** — the spec must list the canonical rules governing the touched surface (North Star principles, schema rules, standing repo rules) as verbatim quotes + testable conditions. Read the North Star section for this surface YOURSELF and check the spec's invariant list against it: a governing principle the spec omits is a High finding (axis: spec).\n3. **Canon fidelity** — verify every canon quote in the spec against the actual file. A claim labeled canonical that does not match the cited file is a High finding (axis: spec) — fabricated canon is the worst drift vector.\n\nTHEN audit alignment across project-north-star, baton-principles, how-we-do-things, spec, and roadmap. Locate the North Star and roadmap (report exactly what you found). Judge neutrally. DRIFT on genuine Critical/High/MEDIUM misalignment (scope creep, direction change, rule violation, off-roadmap work, any silent data loss / destructive action / skipped-staging risk) — but every blocking finding MUST cite concrete evidence (file/section + quoted intent) or be downgraded to Optional. ROADMAP-MISSING only when the absence of a roadmap is the sole blocking issue; otherwise ALIGNED. Do not over-block a correct, minimal, on-roadmap plan, and do not rubber-stamp drift.`,
...
if (shipped && wantValidate) {
  ...
  validation = await agent(
    `You are the pnk-baton VALIDATOR (pre-merge pass).\n${fields}${envNote}\n\nRun the feature end-to-end against the ${env} infrastructure on ${branch}. Judge output against the success criteria — wrong data with exit 0 is a FAIL.\n\nALSO validate the spec's INVARIANTS live (read the spec's "Invariants" section): the rules the change must NOT break must hold on the REAL output, not just in unit tests — e.g. if an invariant says every returned item carries a citation, count citation-less items in the actual response; one violation is a FAIL exactly like a failed success criterion. Report each invariant's observed-vs-expected alongside the criteria.${isProd ? ' This is a PRODUCTION-targeted run: validation is MANDATORY — SKIPPED is NOT acceptable (a SKIP blocks the ship). Only PASS clears it.' : ' If the infrastructure is genuinely unavailable, report SKIPPED with the reason; do not fake a pass.'}\n\nSuccess criteria:\n${plan.successCriteria.map((c, n) => `${n + 1}. ${c}`).join('\n')}`,
