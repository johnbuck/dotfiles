---
description: Run a spec through the baton multi-agent build pipeline (plan → test → build → review → optional validate)
argument-hint: <spec-path> [--validate] [--base <branch>]
---

Run the **baton** build pipeline on the spec/task at: `$ARGUMENTS`
...
2. Invoke the **Workflow** tool with `name: "baton"` and `args: { spec, repo, base, validate }`.
3. When it returns, relay the result concisely: the feature branch name, the gate outcomes (plan criteria count, tests, review PASS/REJECT, validation), and the exact merge command from the `note` field. Do NOT merge automatically — baton produces a reviewed branch; the user ships it.
