...
// the unminified `axe.source`). Same engine; it is injected into the page via
// page.evaluate on every audit, so the smaller payload is leaner. We read the
// file directly (not `import axe`) and cache it. axe-core stays a normal
...
// createRunnerFromConfig — pick the provider. Default is "cdp" (attach to a
// standing CDP endpoint); "agentcore" opts into the per-audit AWS session.
